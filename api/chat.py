...
from services.i18n import ai_language_instruction, localized_text as _localized
...
# literals were simply never covered by that earlier pass. `_localized` is
# now services/i18n.py's shared `localized_text()` helper (promoted there
# during the follow-up site-wide audit so every other endpoint with the same
# bug reuses one fallback chain instead of copy-pasting this docstring).
