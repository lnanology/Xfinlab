...
function u(){
  ...
  var d=e[n],lb=l.querySelector(".xfl-step-label"),ic=l.querySelector(".xfl-step-icon");
  if(lb&&d)lb.textContent=_t(d.key,d.label);
  if(ic&&d)ic.textContent=d.icon;
}
...
window.startAiLoading=function s(){try{if("loading"===document.readyState)return void document.addEventListener("DOMContentLoaded",s,{once:!0});!function(){if(!i){i=!0;isPaid="free"!==getPlan();var st=document.createElement("style");st.textContent='#xfl-ai-loading-overlay{position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:rgba(17,17,17,0.45);backdrop-filter:blur(3px);opacity:0;transition:opacity .2s ease;pointer-events:none}#xfl-ai-loading-overlay.show{opacity:1;pointer-events:auto}#xfl-ai-loading-card{background:var(--bg-card,#fff);border-radius:16px;padding:1.75rem 2rem;width:min(360px,88vw);box-shadow:0 20px 60px rgba(0,0,0,0.25);font-family:"Inter",sans-serif;text-align:center}#xfl-ai-loading-card .xfl-head{font-size:0.8rem;font-weight:600;letter-spacing:.02em;color:var(--accent-blue,#2563EB);margin-bottom:.9rem;display:flex;align-items:center;justify-content:center;gap:.4rem}#xfl-ai-loading-card .xfl-head .xfl-dot{width:7px;height:7px;border-radius:50%;background:var(--accent-blue,#2563EB);animation:xfl-pulse 1.1s ease-in-out infinite}.xfl-anim-wrap{position:relative;width:100%;height:140px;display:flex;align-items:center;justify-content:center;margin-bottom:.9rem}.xfl-quantum-canvas{width:200px;height:140px}.xfl-pulse-rings{position:relative;width:64px;height:64px}.xfl-pulse-rings span{position:absolute;inset:0;border-radius:50%;border:2px solid var(--accent-blue,#2563EB);opacity:0;animation:xfl-ring 2.1s ease-out infinite}.xfl-pulse-rings span:nth-child(2){animation-delay:.7s}.xfl-pulse-rings span:nth-child(3){animation-delay:1.4s}.xfl-pulse-rings .xfl-pulse-core{position:absolute;left:50%;top:50%;width:14px;height:14px;margin:-7px 0 0 -7px;border-radius:50%;background:var(--accent-blue,#2563EB)}.xfl-step-ticker{display:flex;align-items:center;justify-content:center;gap:.5rem;font-size:0.88rem;color:var(--text-primary,#111);font-weight:500}.xfl-step-ticker .xfl-step-icon{font-size:1.1em}@keyframes xfl-pulse{0%,100%{opacity:1}50%{opacity:.3}}@keyframes xfl-ring{0%{transform:scale(0.4);opacity:.6}100%{transform:scale(1.6);opacity:0}}';document.head.appendChild(st);var ov=document.createElement("div");ov.id="xfl-ai-loading-overlay";var animHtml=isPaid?'<canvas class="xfl-quantum-canvas" width="200" height="140"></canvas>':'<div class="xfl-pulse-rings"><span></span><span></span><span></span><span class="xfl-pulse-core"></span></div>';ov.innerHTML='<div id="xfl-ai-loading-card"><div class="xfl-head"><span class="xfl-dot"></span><span class="xfl-head-text">AI 正在分析緊真實市場數據</span></div><div class="xfl-anim-wrap">'+animHtml+'</div><div class="xfl-step-ticker"><span class="xfl-step-icon"></span><span class="xfl-step-label"></span></div></div>',document.body.appendChild(ov),l=ov}}(),u(),a++,o&&(clearTimeout(o),o=null),1===a&&(n=0,c(),l.classList.add("show"),t=setInterval(r,1100),isPaid&&!cvStop&&function(){var cv=l.querySelector(".xfl-quantum-canvas");cv&&(cvStop=startCanvasAnim(cv))}())}catch(e){}},window.stopAiLoading=function(){try{0===(a=Math.max(0,a-1))&&l&&(l.classList.remove("show"),t&&(clearInterval(t),t=null),cvStop&&(cvStop(),cvStop=null),o=setTimeout(function(){n=0},250))}catch(e){}}
