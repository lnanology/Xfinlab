...
_LICENSES: Dict[str, LicenseRecord] = {
    ...
    "gdelt": LicenseRecord(
        ...
        risk_level="low",
        notes=(
            "GDELT's own terms (re-verified by direct fetch of "
            "gdeltproject.org/about.html 2026-08-11): all datasets "
            "'available for unlimited and unrestricted use for any "
            "academic, commercial, or governmental use of any kind "
            "without fee.' No API key, no rate-limit-driven commercial "
            "gate like Polygon/Twelve Data/Tiingo/EODHD's free tiers "
            "above. NOTE (2026-08-11 precision fix): the same terms page "
            "also states redistribution 'must include a citation to the "
            "GDELT Project and a link to this website' -- an attribution "
            "condition, not the zero-condition use a source like "
            "sec_edgar (US government work) gets. 'public_domain' is the "
            "closest fit among this registry's 4-value license_type "
            "enum, but it's not literally unconditional -- filed here as "
            "the best-fit bucket, not a claim of zero obligations. This "
            "codebase already satisfies the condition: XFINLAB's own "
            "output that surfaces GDELT-derived headlines always "
            "attributes 'source' per item and links back via `link`, "
            "same minimal-retention convention as rss_news_service.py "
            "(title/link/published_at/source/language only, never full "
            "article text). Used by services/gdelt_news_service.py "
            "(GDELT 2.0 DOC API, article-level search) and wired into "
            "services/global_news_region_service.py as an additional "
            "per-region source alongside the existing RSS pool."
        ),
    ),
    "fred": LicenseRecord(
        source_id="fred",
